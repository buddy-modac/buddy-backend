...
| 3. 속도·모델 고르기 | 어떤 AI를 쓰고 답을 얼마나 빨리 보낼지 | 용도별 AI 구분 · 체감 9.3초 → 0.7초 |
...
만드는 과정은 이렇다. 먼저 성격에서 그 사람이 **가장 자주 쓰는 사고 습관**을 꺼내고(예: INTP라면 "논리를 정밀하게 따지는 성향"이 1순위), 그 습관이 중시하는 가치들을 조합한다. 거기서 그 성격 특유의 **말투와 기질을 묘사하는 글**이 만들어진다. 이 묘사 글이 결과물의 핵심이다.
...
여기에 두 가지 스위치가 있다. 하나는 **말투를 켜고 끄는 스위치**로, 끄면 ②가 빠져서 담백한 기본 모드가 된다. 다른 하나는, 성격 말투가 답의 정확성을 해칠 것 같으면 **AI가 스스로 "이번엔 기본 모드가 낫겠다"고 신호를 보내는 장치**다. "정확함이 말투보다 우선"이라는 원칙을 말로만이 아니라 실제로 지키게 하는 안전장치다.
...
성격을 살리는 것과 비서로서의 정확함을 둘 다 지키는 문제는 아직 완전히 풀지 못한 숙제로 남았다.
...
다음은 **답을 얼마나 빨리 보내느냐** 실험이다. 사용자가 답을 받기까지 체감하는 대기 시간을 네 가지 방식으로 비교했다.

![3단계 전송 방식별 대기 시간](assets/05_phase3_latency.png)

AI 모델만 더 가벼운 걸로 바꿔도 9.3초에서 6.6초로 줄지만 여전히 길다. **답을 한 번에 다 만든 뒤 보내지 말고, 글자가 만들어지는 대로 흘려보내는 방식("스트리밍")이 압도적으로 빨랐다** — 첫 글자가 0.7초 만에 뜬다. 미리 답을 만들어두는 방식도 시도했지만, 스트리밍만 못한 데다 비용까지 더 들어서 추천하지 않기로 했다.

---

## 한 줄 정리

AI에게 16가지 성격 말투를 입히고 그게 잘 통하는지 점수로 확인하는 시스템을 만들었다. 성격 질문에서는 거의 완벽(88→99%)하게 다듬었고, 훨씬 어려운 비서 상황에서는 "행동으로 보여주는" 방식으로 12→51%까지 끌어올렸으며, 마지막엔 용도별 AI 선택과 빠른 전송 방식(9.3초→0.7초)까지 정리했다. 아직 못 푼 숙제는 **외향/내향 구분**과, **성격을 살리면서 정확함도 그대로 지키는 것** 두 가지다.
