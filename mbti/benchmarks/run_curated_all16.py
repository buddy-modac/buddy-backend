...
REPORT = os.path.join(HERE, "curated_all16_20260626.md")
...
def main():
    ...
    open(REPORT, "w").close()
    w(f"# Phase B — 큐레이션 행동형 16유형 (실사용 probe, K={K})\n")
    ...
    with open(os.path.join(HERE, "RESULTS_LOG.md"), "a", encoding="utf-8") as f:
        f.write(f"\n---\n\n## {stamp} · \"Option A 행동형 16유형\"\n- 정확 {100*exact_t//N}%(vs12) "
                f"패밀리 {100*fam_t//N}%(vs25) P등록 {preg_t}/{N} · 상세 curated_all16_20260626.md\n")
