"""지연 측정(explain) — explain(이미지/텍스트, 모델, 출력길이)별 TTFT·총시간 비교.
...
def user_msg(concise):
    base = (f"요청: 아래 이미지와 텍스트의 의미를 설명해 주세요.\n\n[추출된 텍스트]\n{OCR}")
    if concise:
        base += "\n\n핵심만 간결하게 2~3문장으로. 표·긴 목록·부연 없이."
    return base
...
def main():
    ...
    configs = [
        ("① 전(현재): Sonnet+이미지+2000(verbose)", SONNET, True, 2000, False),
        ("② 이미지 제거: Sonnet + 텍스트만 + 2000", SONNET, False, 2000, False),
        ("③ +Haiku: Haiku + 텍스트만 + 2000",      HAIKU,  False, 2000, False),
        ("④ +간결: Haiku+텍스트만+700+간결", HAIKU,  False, 400,  True),
    ]
