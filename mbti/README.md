...
**이미지 클립보드 API** (공개): `POST /clipboard`(멀티파트 `file=` 또는 JSON `{image_b64,media_type,name}`) · `GET /clipboard`(목록, 각 항목에 `url`) · `GET /clipboard/{id}/raw`(원본 바이트, `?download=1`로 다운로드) · `DELETE /clipboard/{id}`. 프런트는 받은 `url`을 `<img src>`에 그대로 쓰면 됨(디코딩 불필요). 업로드 이미지는 `server/uploads/`에 저장(gitignore).
