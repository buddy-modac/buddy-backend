...
> **다른 기기에 띄운 서버의 `/admin` 을 내 PC에서 보려면:** 기본은 localhost 전용(외부 403)입니다. 서버 실행 기기에서 `SERVER_ADMIN_ALLOW_IPS` 에 **내 PC의 IP**를 추가하세요 (`server/.env.local` 또는 환경변수, 콤마 구분·CIDR 가능). 차단 시 403 응답에 **현재 접속 IP**가 찍히니 그 값을 그대로 넣으면 됩니다. 예: `SERVER_ADMIN_ALLOW_IPS=192.168.0.50`.
